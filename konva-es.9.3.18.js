...
function getNumberOrAutoValidator() {
  if (Konva.isUnminified)
    return function(val, attr) {
      ...
      return isNumber || isAuto || Util.warn(
  ...
  _sceneFunc(context) {
    ...
    for (direction === RTL && context.setAttr("direction", direction), context.setAttr("font", this._getContextFont()), context.setAttr("textBaseline", baseline), context.setAttr("textAlign", LEFT2), verticalAlign === MIDDLE ? alignY = (this.getHeight() - textArrLen * lineHeightPx - padding * 2) / 2 : verticalAlign === BOTTOM && (alignY = this.getHeight() - textArrLen * lineHeightPx - padding * 2), context.translate(padding, alignY + padding), n = 0; n < textArrLen; n++) {
      ...
      if (shouldLineThrough) {
        ...
        context.moveTo(lineTranslateX, translateY + lineTranslateY + yOffset), spacesNumber = text.split(" ").length - 1, oneWord = spacesNumber === 0, lineWidth = align === JUSTIFY && !lastLine ? totalWidth - padding * 2 : width, context.lineTo(
    ...
    for (let i = 0, max = lines.length; i < max; ++i) {
      ...
      if (fixedWidth && lineWidth > maxWidth)
        for (; line.length > 0; ) {
          ...
          for (; low < high; ) {
            let mid = low + high >>> 1, lineArray = stringToArray(line), substr = lineArray.slice(0, mid + 1).join(""), substrWidth = this._getTextWidth(substr) + additionalWidth;
...
var Enhance = function(imageData) {
  ...
  enhanceAmount > 0 ? (rGoalMax = rMax + enhanceAmount * (255 - rMax), rGoalMin = rMin - enhanceAmount * (rMin - 0), gGoalMax = gMax + enhanceAmount * (255 - gMax), gGoalMin = gMin - enhanceAmount * (gMin - 0), bGoalMax = bMax + enhanceAmount * (255 - bMax), bGoalMin = bMin - enhanceAmount * (bMin - 0)) : (rMid = (rMax + rMin) * 0.5, rGoalMax = rMax + enhanceAmount * (rMax - rMid), rGoalMin = rMin + enhanceAmount * (rMin - rMid), gMid = (gMax + gMin) * 0.5, gGoalMax = gMax + enhanceAmount * (gMax - gMid), gGoalMin = gMin + enhanceAmount * (gMin - gMid), bMid = (bMax + bMin) * 0.5, bGoalMax = bMax + enhanceAmount * (bMax - bMid), bGoalMin = bMin + enhanceAmount * (bMin - bMid));
  for (let i = 0; i < nSubPixels; i += 4)
