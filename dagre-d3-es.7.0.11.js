...
function incrementOrInitEntry(map11, k) {
  if (map11[k]) {
    map11[k]++;
  } else {
    map11[k] = 1;
...
function decrementOrRemoveEntry(map11, k) {
  if (!--map11[k]) {
    delete map11[k];
  }
}
...
function run(g) {
  ...
  _3.forEach(fas, function(e) {
    var label = g.edge(e);
    g.removeEdge(e);
    label.forwardName = e.name;
    label.reversed = true;
    g.setEdge(e.w, e.v, label, _3.uniqueId("rev"));
  });
...
function dfsFAS(g) {
  ...
  function dfs3(v) {
    ...
    visited[v] = true;
    stack[v] = true;
    _3.forEach(g.outEdges(v), function(e) {
...
function buildLayerMatrix(g) {
  var layering = _4.map(_4.range(maxRank(g) + 1), function() {
    return [];
  });
  _4.forEach(g.nodes(), function(v) {
    var node = g.node(v);
    var rank2 = node.rank;
    if (!_4.isUndefined(rank2)) {
...
function removeEmptyRanks(g) {
  ...
  _4.forEach(layers, function(vs, i) {
    if (_4.isUndefined(vs) && i % nodeRankFactor !== 0) {
      --delta;
    } else if (delta) {
      _4.forEach(vs, function(v) {
        g.node(v).rank += delta;
      });
    }
  });
...
function partition(collection, fn) {
  var result = { lhs: [], rhs: [] };
  _4.forEach(collection, function(value) {
    if (fn(value)) {
...
function time(name, fn) {
  ...
  try {
    ...
    console.log(name + " time: " + (_4.now() - start) + "ms");
  }
}
...
function swapWidthHeight(g) {
  _6.forEach(g.nodes(), function(v) {
    swapWidthHeightOne(g.node(v));
  });
  _6.forEach(g.edges(), function(e) {
...
function reverseY(g) {
  ...
  _6.forEach(g.edges(), function(e) {
    var edge = g.edge(e);
    _6.forEach(edge.points, reverseYOne);
...
function swapXY(g) {
  ...
  _6.forEach(g.edges(), function(e) {
    var edge = g.edge(e);
    _6.forEach(edge.points, swapXYOne);
...
function run2(g) {
  g.graph().dummyChains = [];
  _7.forEach(g.edges(), function(edge) {
...
function undo3(g) {
  _7.forEach(g.graph().dummyChains, function(v) {
    var node = g.node(v);
...
function longestPath(g) {
  ...
  function dfs3(v) {
    ...
    visited[v] = true;
    var rank2 = _8.min(
      _8.map(g.outEdges(v), function(e) {
...
function tightTree(t, g) {
  function dfs3(v) {
    _9.forEach(g.nodeEdges(v), function(e) {
  ...
  _9.forEach(t.nodes(), dfs3);
  return t.nodeCount();
}
function findMinSlackEdge(t, g) {
  return _9.minBy(g.edges(), function(e) {
...
var DEFAULT_WEIGHT_FUNC = _11.constant(1);
...
var DEFAULT_WEIGHT_FUNC2 = _14.constant(1);
...
function topsort(g) {
  ...
  function visit(node) {
    ...
    if (!Object.prototype.hasOwnProperty.call(visited, node)) {
      stack[node] = true;
      visited[node] = true;
      _15.each(g.predecessors(node), visit);
      delete stack[node];
      results.push(node);
    }
  }
  _15.each(g.sinks(), visit);
  if (_15.size(visited) !== g.nodeCount()) {
    throw new CycleException();
  }
  return results;
}
...
function doDfs(g, v, postorder3, visited, navigation, acc) {
  if (!Object.prototype.hasOwnProperty.call(visited, v)) {
    ...
    _16.each(navigation(v), function(w) {
...
function calcCutValue(t, g, child) {
  ...
  _18.forEach(g.nodeEdges(child), function(e) {
...
function dfsAssignLowLim(tree, visited, nextLim, v, parent) {
  ...
  visited[v] = true;
  _18.forEach(tree.neighbors(v), function(w) {
...
function leaveEdge(tree) {
  return _18.find(tree.edges(), function(e) {
...
function enterEdge(t, g, edge) {
  ...
  var candidates = _18.filter(g.edges(), function(edge2) {
    return flip === isDescendant(t, t.node(edge2.v), tailLabel) && flip !== isDescendant(t, t.node(edge2.w), tailLabel);
  });
  return _18.minBy(candidates, function(edge2) {
...
function updateRanks(t, g) {
  var root = _18.find(t.nodes(), function(v) {
...
function run3(g) {
  ...
  var height = _19.max(_19.values(depths)) - 1;
  var nodeSep = 2 * height + 1;
  g.graph().nestingRoot = root;
  _19.forEach(g.edges(), function(e) {
    g.edge(e).minlen *= nodeSep;
  });
  var weight = sumWeights(g) + 1;
  _19.forEach(g.children(), function(child) {
...
function dfs2(g, root, nodeSep, weight, height, depths, v) {
  ...
  _19.forEach(children, function(child) {
    dfs2(g, root, nodeSep, weight, height, depths, child);
...
function treeDepths(g) {
  ...
  _19.forEach(g.children(), function(v) {
    dfs3(v, 1);
  });
  return depths;
}
...
function buildLayerGraph(g, rank2, relationship) {
  ...
  _21.forEach(g.nodes(), function(v) {
    var node = g.node(v), parent = g.parent(v);
    if (node.rank === rank2 || node.minRank <= rank2 && rank2 <= node.maxRank) {
      result.setNode(v);
      result.setParent(v, parent || root);
      _21.forEach(g[relationship](v), function(e) {
        var u = e.v === v ? e.w : e.v, edge = result.edge(u, v), weight = !_21.isUndefined(edge) ? edge.weight : 0;
...
function createRootNode(g) {
  var v;
  while (g.hasNode(v = _21.uniqueId("_root"))) ;
  return v;
}
...
function twoLayerCrossCount(g, northLayer, southLayer) {
  var southPos = _22.zipObject(
    southLayer,
    _22.map(southLayer, function(v, i) {
      return i;
    })
  );
  var southEntries = _22.flatten(
    _22.map(northLayer, function(v) {
      return _22.sortBy(
        _22.map(g.outEdges(v), function(e) {
  ...
  firstIndex -= 1;
  var tree = _22.map(new Array(treeSize), function() {
    return 0;
  });
  var cc = 0;
  _22.forEach(
...
function initOrder(g) {
  var visited = {};
  var simpleNodes = _23.filter(g.nodes(), function(v) {
    return !g.children(v).length;
  });
  var maxRank2 = _23.max(
    _23.map(simpleNodes, function(v) {
      return g.node(v).rank;
    })
  );
  var layers = _23.map(_23.range(maxRank2 + 1), function() {
    return [];
  });
  function dfs3(v) {
    if (_23.has(visited, v)) return;
    visited[v] = true;
    var node = g.node(v);
    layers[node.rank].push(v);
    _23.forEach(g.successors(v), dfs3);
  }
  var orderedVs = _23.sortBy(simpleNodes, function(v) {
    return g.node(v).rank;
  });
  _23.forEach(orderedVs, dfs3);
  return layers;
}
...
function barycenter(g, movable) {
  return _24.map(movable, function(v) {
    var inV = g.inEdges(v);
    if (!inV.length) {
      return { v };
    } else {
      var result = _24.reduce(
        inV,
        function(acc, e) {
...
function resolveConflicts(entries, cg) {
  var mappedEntries = {};
  _25.forEach(entries, function(entry, i) {
    ...
    if (!_25.isUndefined(entry.barycenter)) {
      tmp.barycenter = entry.barycenter;
      tmp.weight = entry.weight;
    }
  });
  _25.forEach(cg.edges(), function(e) {
    var entryV = mappedEntries[e.v];
    var entryW = mappedEntries[e.w];
    if (!_25.isUndefined(entryV) && !_25.isUndefined(entryW)) {
      entryW.indegree++;
      entryV.out.push(mappedEntries[e.w]);
    }
  });
  var sourceSet = _25.filter(mappedEntries, function(entry) {
...
function doResolveConflicts(sourceSet) {
  ...
  function handleIn(vEntry) {
    return function(uEntry) {
      ...
      if (_25.isUndefined(uEntry.barycenter) || _25.isUndefined(vEntry.barycenter) || uEntry.barycenter >= vEntry.barycenter) {
  ...
  while (sourceSet.length) {
    ...
    entries.push(entry);
    _25.forEach(entry["in"].reverse(), handleIn(entry));
    _25.forEach(entry.out, handleOut(entry));
  }
  return _25.map(
    _25.filter(entries, function(entry2) {
      return !entry2.merged;
    }),
    function(entry2) {
      return _25.pick(entry2, ["vs", "i", "barycenter", "weight"]);
    }
  );
}
...
function sort(entries, biasRight) {
  ...
  var sortable = parts.lhs, unsortable = _26.sortBy(parts.rhs, function(entry) {
    return -entry.i;
  }), vs = [], sum = 0, weight = 0, vsIndex = 0;
  sortable.sort(compareWithBias(!!biasRight));
  vsIndex = consumeUnsortable(vs, unsortable, vsIndex);
  _26.forEach(sortable, function(entry) {
...
function consumeUnsortable(vs, unsortable, index) {
  var last4;
  while (unsortable.length && (last4 = _26.last(unsortable)).i <= index) {
    unsortable.pop();
    vs.push(last4.vs);
    index++;
  }
...
function sortSubgraph(g, v, cg, biasRight) {
  ...
  var subgraphs = {};
  if (bl) {
    movable = _27.filter(movable, function(w) {
      return w !== bl && w !== br;
    });
  }
  var barycenters = barycenter(g, movable);
  _27.forEach(barycenters, function(entry) {
  ...
  if (bl) {
    result.vs = _27.flatten([bl, result.vs, br]);
...
function expandSubgraphs(entries, subgraphs) {
  _27.forEach(entries, function(entry) {
    entry.vs = _27.flatten(
...
function mergeBarycenters(target, other) {
  if (!_27.isUndefined(target.barycenter)) {
...
// src/dagre/order/index.js
function order(g) {
  var maxRank2 = maxRank(g), downLayerGraphs = buildLayerGraphs(g, _28.range(1, maxRank2 + 1), "inEdges"), upLayerGraphs = buildLayerGraphs(g, _28.range(maxRank2 - 1, -1, -1), "outEdges");
...
function buildLayerGraphs(g, ranks, relationship) {
  return _28.map(ranks, function(rank2) {
    return buildLayerGraph(g, rank2, relationship);
  });
}
function sweepLayerGraphs(layerGraphs, biasRight) {
  var cg = new Graph();
  _28.forEach(layerGraphs, function(lg) {
    var root = lg.graph().root;
    var sorted = sortSubgraph(lg, root, cg, biasRight);
    _28.forEach(sorted.vs, function(v, i) {
      lg.node(v).order = i;
    });
...
function assignOrder(g, layering) {
  _28.forEach(layering, function(layer) {
    _28.forEach(layer, function(v, i) {
      g.node(v).order = i;
    });
  });
}
...
function parentDummyChains(g) {
  ...
  _29.forEach(g.graph().dummyChains, function(v) {
    var node = g.node(v);
...
function postorder2(g) {
  ...
  function dfs3(v) {
    var low = lim;
    _29.forEach(g.children(v), dfs3);
    result[v] = { low, lim: lim++ };
  }
  _29.forEach(g.children(), dfs3);
  return result;
}
...
function findType1Conflicts(g, layering) {
  ...
  function visitLayer(prevLayer, layer) {
    var k0 = 0, scanPos = 0, prevLayerLength = prevLayer.length, lastNode = _30.last(layer);
    _30.forEach(layer, function(v, i) {
      var w = findOtherInnerSegmentNode(g, v), k1 = w ? g.node(w).order : prevLayerLength;
      if (w || v === lastNode) {
        _30.forEach(layer.slice(scanPos, i + 1), function(scanNode) {
          _30.forEach(g.predecessors(scanNode), function(u) {
  ...
  _30.reduce(layering, visitLayer);
  return conflicts;
}
function findType2Conflicts(g, layering) {
  ...
  function scan(south, southPos, southEnd, prevNorthBorder, nextNorthBorder) {
    var v;
    _30.forEach(_30.range(southPos, southEnd), function(i) {
      v = south[i];
      if (g.node(v).dummy) {
        _30.forEach(g.predecessors(v), function(u) {
          var uNode = g.node(u);
  ...
  function visitLayer(north, south) {
    ...
    _30.forEach(south, function(v, southLookahead) {
...
function findOtherInnerSegmentNode(g, v) {
  if (g.node(v).dummy) {
    return _30.find(g.predecessors(v), function(u) {
...
function verticalAlignment(g, layering, conflicts, neighborFn) {
  var root = {}, align = {}, pos = {};
  _30.forEach(layering, function(layer) {
    _30.forEach(layer, function(v, order2) {
      root[v] = v;
      align[v] = v;
      pos[v] = order2;
    });
  ...
  _30.forEach(layering, function(layer) {
    var prevIdx = -1;
    _30.forEach(layer, function(v) {
      var ws = neighborFn(v);
      if (ws.length) {
        ws = _30.sortBy(ws, function(w2) {
          return pos[w2];
        });
...
function findSmallestWidthAlignment(g, xss) {
  return _30.minBy(_30.values(xss), function(xs) {
    var max7 = Number.NEGATIVE_INFINITY;
    var min4 = Number.POSITIVE_INFINITY;
    _30.forIn(xs, function(x, v) {
...
function alignCoordinates(xss, alignTo) {
  var alignToVals = _30.values(alignTo), alignToMin = _30.min(alignToVals), alignToMax = _30.max(alignToVals);
  _30.forEach(["u", "d"], function(vert) {
    _30.forEach(["l", "r"], function(horiz) {
      var alignment = vert + horiz, xs = xss[alignment], delta;
      if (xs === alignTo) return;
      var xsVals = _30.values(xs);
      delta = horiz === "l" ? alignToMin - _30.min(xsVals) : alignToMax - _30.max(xsVals);
      if (delta) {
        xss[alignment] = _30.mapValues(xs, function(x) {
          return x + delta;
        });
      }
    });
...
function balance(xss, align) {
  return _30.mapValues(xss.ul, function(ignore, v) {
    if (align) {
      return xss[align.toLowerCase()][v];
    } else {
      var xs = _30.sortBy(_30.map(xss, v));
      return (xs[1] + xs[2]) / 2;
    }
  });
}
function positionX(g) {
  ...
  var conflicts = _30.merge(findType1Conflicts(g, layering), findType2Conflicts(g, layering));
  var xss = {};
  var adjustedLayering;
  _30.forEach(["u", "d"], function(vert) {
    adjustedLayering = vert === "u" ? layering : _30.values(layering).reverse();
    _30.forEach(["l", "r"], function(horiz) {
      if (horiz === "r") {
        adjustedLayering = _30.map(adjustedLayering, function(inner) {
          return _30.values(inner).reverse();
        });
      }
      ...
      if (horiz === "r") {
        xs = _30.mapValues(xs, function(x) {
          return -x;
        });
      }
      xss[vert + horiz] = xs;
...
function position(g) {
  ...
  positionY(g);
  _31.forOwn(positionX(g), function(x, v) {
    g.node(v).x = x;
  });
}
function positionY(g) {
  ...
  _31.forEach(layering, function(layer) {
    var maxHeight = _31.max(
      _31.map(layer, function(v) {
...
function updateInputGraph(inputGraph, layoutGraph) {
  _32.forEach(inputGraph.nodes(), function(v) {
  ...
  _32.forEach(inputGraph.edges(), function(e) {
...
function buildLayoutGraph(inputGraph) {
  ...
  g.setGraph(
    _32.merge({}, graphDefaults, selectNumberAttrs(graph, graphNumAttrs), _32.pick(graph, graphAttrs))
  );
  _32.forEach(inputGraph.nodes(), function(v) {
    var node = canonicalize(inputGraph.node(v));
    g.setNode(v, _32.defaults(selectNumberAttrs(node, nodeNumAttrs), nodeDefaults));
  ...
  _32.forEach(inputGraph.edges(), function(e) {
    ...
    g.setEdge(
      e,
      _32.merge({}, edgeDefaults, selectNumberAttrs(edge, edgeNumAttrs), _32.pick(edge, edgeAttrs))
    );
  });
  return g;
}
function makeSpaceForEdgeLabels(g) {
  var graph = g.graph();
  graph.ranksep /= 2;
  _32.forEach(g.edges(), function(e) {
    var edge = g.edge(e);
    edge.minlen *= 2;
...
function assignRankMinMax(g) {
  var maxRank2 = 0;
  _32.forEach(g.nodes(), function(v) {
    var node = g.node(v);
    if (node.borderTop) {
      node.minRank = g.node(node.borderTop).rank;
      node.maxRank = g.node(node.borderBottom).rank;
      maxRank2 = _32.max(maxRank2, node.maxRank);
    }
  });
...
function translateGraph(g) {
  ...
  _32.forEach(g.edges(), function(e) {
    var edge = g.edge(e);
    _32.forEach(edge.points, function(p) {
      p.x -= minX;
      p.y -= minY;
    });
    if (Object.prototype.hasOwnProperty.call(edge, "x")) {
...
function removeBorderNodes(g) {
  _32.forEach(g.nodes(), function(v) {
    if (g.children(v).length) {
      ...
      var l = g.node(_32.last(node.borderLeft));
      var r = g.node(_32.last(node.borderRight));
...
function insertSelfEdges(g) {
  ...
  _32.forEach(layers, function(layer) {
    var orderShift = 0;
    _32.forEach(layer, function(v, i) {
      var node = g.node(v);
      node.order = i + orderShift;
      _32.forEach(node.selfEdges, function(selfEdge) {
        addDummyNode(
          g,
          "selfedge",
          {
...
function selectNumberAttrs(obj, attrs) {
  return _32.mapValues(_32.pick(obj, attrs), Number);
}
function canonicalize(attrs) {
  var newAttrs = {};
  _32.forEach(attrs, function(v, k) {
...
function applyTransition(selection, g) {
  var graph = g.graph();
  if (_33.isPlainObject(graph)) {
    var transition = graph.transition;
    if (_33.isFunction(transition)) {
...
var createEdgePaths = function(selection, g, arrows2) {
  ...
  svgPaths.selectAll("path.path").each(function(e) {
    var edge = g.edge(e);
    edge.arrowheadId = _34.uniqueId("arrowhead");
...
function enter(svgPaths, g) {
  ...
  svgPathsEnter.append("path").attr("class", "path").attr("d", function(e) {
    ...
    var points = _34.range(edge.points.length).map(function() {
